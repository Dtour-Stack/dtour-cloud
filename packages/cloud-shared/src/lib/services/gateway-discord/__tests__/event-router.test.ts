...
describe("sanitizeError logic", () => {
  ...
  const sampleToken = (id: string, timestamp: string, hmac: string) =>
    `${id}.${timestamp}.${hmac}`;

  const SAMPLE_TOKENS = [
    sampleToken(
      "MTE0NzU5OTI0NzM5NjEyNjcyMA",
      "G1xK2z",
      "FAKE_TOKEN_abcdefghijklmnopqrstuvwxyz12",
    ),
    sampleToken(
      "MTE0NzU5OTI0NzM5NjEyNjcyMQ",
      "H2yL3a",
      "TEST_TOKEN_ABCDEFGHIJKLMNOPQRSTUVWXYZ34",
    ),
    sampleToken(
      "MTE0NzU5OTI0NzM5NjEyNjcyMg",
      "I3zM4b",
      "MOCK_TOKEN_0123456789abcdefghijklmno56",
    ),
  ];
