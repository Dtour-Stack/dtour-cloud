...
type ProvisionContextResult = {
  caller: { pubkey: string; role: Role };
  agent: {
    id: Id<"agents">;
    name: string;
...
export const provisionContext = internalQuery({
  ...
  handler: async (ctx, { token, agentId }): Promise<ProvisionContextResult> => {
...
export const deploy = action({
  ...
  handler: async (ctx, { token, agentId }) => {
    const context = await ctx.runQuery(provisionContextRef, { token, agentId });
...
export const sync = action({
  ...
  handler: async (ctx, { token, agentId }) => {
    ...
    if (!upstreamAgentId) throw new Error("Deploy the remote runtime before syncing status");
    ...
    return await ctx.runMutation(recordProvisionStateRef, {
      token,
      agentId,
  ...
  handler: async (ctx, { token, agentId }) => {
    ...
    if (status === 202) {
      ...
      return {
        ...
        retryAfterMs: numberProp(data, "retryAfterMs") ?? 5000,
        message:
...
export const suspend = action({
  ...
  handler: async (ctx, { token, agentId }) => {
    ...
    const { payload } = await elizaFetch(
      "POST",
      `/api/v1/eliza/agents/${encodeURIComponent(upstreamAgentId)}/suspend`,
    );
    const data = recordProp(payload, "data");
