...
import { bootstrapCodingSandbox, launchCommandForAgent } from "./bootstrap";
import {
	DeviceRegistry,
	decodeFromDevice,
	encodeToDevice,
...
async function runnerOpen(ws: ServerWebSocket<WSData>, data: RunnerWSData) {
	...
	try {
		...
		await sandbox.pty.sendInput(
			term.pid,
			enc.encode(`cd ~/workspace && ${launchCommandForAgent(data.agent)}\r`),
		);
	} catch (e) {
		ws.send(
...
Bun.serve<WSData>({
	...
	async fetch(req, server) {
		...
		if (
			...
			url.pathname.endsWith("/coding-health")
		) {
		...
		if (backend === "selfhost") {
			...
			const requestedDeviceId =
				url.searchParams.get("deviceId")?.trim() || null;
			if (
				server.upgrade(req, {
					data: {
						kind: "selfhost",
	...
	websocket: {
		...
		async open(ws) {
			...
			if (data.kind === "device") {
				...
				void convex
					.mutation(anyApi.codingDevices.markDeviceSeen, { token: data.token })
					.catch(() => {});
				return;
			}
			// selfhost web session — bridge to the owner's connected device
			const dev = devices.get(data.pubkey, data.deviceId ?? undefined);
			if (!dev) {
				ws.send(
					data.deviceId
						? "\r\n  \x1b[33mselected detour desktop is not connected\x1b[0m — open the Detour app and\r\n" +
		...
		async close(ws) {
			...
			if (data.kind === "device") {
				data.closed = true;
				devices.unregister(data.pubkey, data.deviceId);
				// Tear down any web sessions that were bridged to this device.
				for (const [sid, web] of selfhostWeb) {
					if (
						web.data.kind === "selfhost" &&
						web.data.pubkey === data.pubkey &&
						web.data.deviceId === data.deviceId
					) {
						web.close();
						sessions.close(sid);
			...
			devices
				.get(data.pubkey, data.deviceId ?? undefined)
				?.send(encodeToDevice({ t: "close", sid: data.sid }));
		},
	},
});
