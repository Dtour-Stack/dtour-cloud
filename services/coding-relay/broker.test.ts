...
describe("device frame codec", () => {
  ...
  it("decodes a device→relay output frame", () => {
    expect(decodeFromDevice(JSON.stringify({ t: "out", sid: "s1", data: "hi" }))).toEqual({
      t: "out",
      sid: "s1",
      data: "hi",
    });
...
describe("parseWebFrame", () => {
  ...
  it("parses a resize frame", () => {
    expect(parseWebFrame(`r${JSON.stringify({ cols: 120, rows: 40 })}`)).toEqual({
      kind: "resize",
      cols: 120,
      rows: 40,
    });
  ...
  it("rejects a non-positive resize", () => {
    expect(parseWebFrame(`r${JSON.stringify({ cols: 0, rows: 40 })}`)).toBeNull();
  });
...
describe("DeviceRegistry", () => {
  const conn = (pubkey: string): DeviceConn => ({ pubkey, deviceId: "d", send: () => {} });
  ...
  it("a new registration for the same owner replaces the old", () => {
    const r = new DeviceRegistry();
