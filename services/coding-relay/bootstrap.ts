...
const HOME_SETUP = `export HOME="\${HOME:-/home/user}" && mkdir -p "$HOME" "$HOME/workspace" "$HOME/.detour" "$HOME/.detour/bin"`;
...
export function launchCommandForAgent(agentId: string): string {
	const id = agentId in LAUNCH_CMD ? agentId : "opencode";
	return LAUNCH_CMD[id] ?? "opencode";
}
...
export async function bootstrapCodingSandbox(
	...
	const launch = launchCommandForAgent(id);
	onProgress(
		`\r\n  \x1b[32mready\x1b[0m — starting \x1b[36m${launch}\x1b[0m · work in ~/workspace\r\n` +
